...
<NodeViewWrapper
	class="code-wrapper border border-black rounded-md"
	draggable={false}
	spellcheck={false}
>
	<div
		class="code-wrapper-tile border-black border-b-1 flex p-2 justify-between"
	...
	>
		<DropdownMenu.Root>
			...
			<DropdownMenu.Content
				side="right"
				align="start"
				class="h-36 w-30 overflow-auto border border-black hide-scrollbar"
				contenteditable="false"
			>
				{#each languages as language (language)}
					<DropdownMenu.Item
						contenteditable="false"
						data-current={defaultLanguage ===
							language}
		...
		<Button
			variant="ghost"
			class="text-muted-foreground size-4 p-0"
			onclick={copyCode}
		>
			{#if isCopying}
	...
	<pre class="px-2" bind:this={preRef} draggable={false}>
		<NodeViewContent
			as="code"
